...
- **Critère essentiel : cohérence du parcours d'ensemble.** Phases bien identifiées sur leurs marqueurs institutionnels et d'homogénéité culturelle, ordre canonique respecté (toute déviation justifiée par une perturbation), saillants aux bons endroits, tous les événements importants expliqués honnêtement. Les durées typiques (~200 ans / 25-80 ans pour RN) sont des références indicatives — les écarts atypiques sont des signaux à interpréter (perturbations identifiées), pas des erreurs à corriger en déplaçant les bornes. Ne jamais forcer une borne pour obtenir une durée typique.
...
- **Pacte oligarchique = nécessité structurelle** entre la coagulation/bascule et l'essor oligarchique. Critère universel : **acte collectif par lequel les oligarques garantissent la stabilité du régime aux successions** — le pouvoir survit désormais au remplacement du suzerain (test discriminant féodal/oligarchique). Deux chemins admis : **chemin A** = codification de la succession (loi salique 1317, abolition co-régence Venise 1032, primogéniture Bavière 1506) ; **chemin B** = codification de l'exercice du pouvoir au point que le pouvoir cesse d'être patrimoine personnel (Ordonnances anglaises 1311). Une nation peut combiner les deux successivement (Venise : A en 1032, déploiement de B avec Minor Consiglio ~1140 et Promissio 1172). Distingue le pacte des créations institutionnelles unilatérales (Philippe le Bel crée le Parlement de Paris, mais c'est un outil du roi — le vrai pacte français est la loi salique). Vérifié sur 10/11 parcours canoniques avec acte attesté ; Israël antique présente les marqueurs structurels d'un PO sous Omri (-885) mais l'acte lui-même n'est pas textuellement attesté (sources rares au IXe siècle av. J.-C.). Ne jamais conclure « absent, résolution diffuse » sans justification structurelle solide appuyée sur un cas comparable (ex. : Venise, pacte institutionnel et non personnel).
...
- **v3 (2026-04-18)** : ajouts méthodologiques — critère formel du pacte oligarchique, guerre sociale enjeu/forme (économique pure / géographique / mixte), acmé oligarchique = saturation exutoire, empire intégré vs union personnelle, correction_echelle vs choc_exogene fin d'exutoire, échelle du test absolutiste, piège de la codification écrite, cas particuliers RN (avortée, contre occupant, AR exogène, IR double, test RN vs révolte absolutiste).
