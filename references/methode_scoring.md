...
6. **Candidats pour perturbations structurelles (prolongement / accélération)**. Repérer dans le dossier les événements qui modifient potentiellement l'**homogénéité effective du corps politique** — accroissement de l'hétérogénéité (typiquement intégration effective d'une marge, exutoire continu) ou augmentation de l'homogénéité / recentrage forcé (typiquement perte d'un territoire digéré, modernisation imposée, fin d'un exutoire militaire). Le canal territorial est le plus fréquent mais pas l'automatique : appliquer le **test de digestion** (administration directe par du personnel du noyau ? droit du noyau imposé ? élites centrales durablement mobilisées ?) avant de qualifier un changement territorial comme perturbation structurelle — une union personnelle laissée autonome n'en est pas une. Pour chaque candidat, formuler une hypothèse de mécanisme + effet, ou signaler explicitement que l'événement n'est pas une perturbation structurelle parce que la digestion n'a pas eu lieu. Ne pas trancher : ce sont des **candidats** que le parent évaluera dans la construction du parcours. Cf. `perturbations.md` §1 « Principe structurel » et §3.
